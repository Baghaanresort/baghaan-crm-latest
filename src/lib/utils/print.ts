...
// to prevent stored XSS in the rendered/printed/dispatched voucher.
...
const PAYMENT_TYPE_LABEL: Record<Payment['type'], string> = {
  advance: 'Advance',
  balance: 'Balance',
  btc_receipt: 'BTC Receipt',
  refund: 'Refund',
};

// ---------- Voucher HTML ----------
// `payments` are the Accounts-verified payments for this booking (oldest-first).
// They drive the live "Payments Received" ledger and the balance. Defaults to []
// so a caller that omits them simply gets a voucher with no transactions section.
export function buildVoucherHTML(b: Booking, payments: Payment[] = []): string {
  ...
  const roomsLine = Object.entries(roomGroups)
    ...
    .join(', ');
  // Live financial picture from verified payments. Refunds reduce the received
  // total. When no verified payments exist we fall back to the booking's stored
  // advancePaid so legacy/dispatched vouchers still show a sensible figure.
  ...
  const paymentsSectionHTML = hasPayments
    ? `<tr><td class="k">Payments Received</td><td>
        <table class="txn">
          <tr><th>Date</th><th>Type / Mode</th><th>Reference</th><th class="r">Amount (Rs.)</th></tr>
          ${payments
            .map((p) => {
              const isRefund = p.type === 'refund';
              const typeLabel = PAYMENT_TYPE_LABEL[p.type] ?? 'Payment';
              return `<tr><td>${esc(fmtDate(p.paymentDate))}</td><td>${typeLabel}${p.mode ? ` · ${esc(p.mode)}` : ''}</td><td>${p.reference ? esc(p.reference) : '—'}</td><td class="r">${isRefund ? '-' : ''}${Number(p.amount).toLocaleString('en-IN')}</td></tr>`;
            })
            .join('')}
          <tr class="txn-total"><td colspan="3">Total Received</td><td class="r">${received.toLocaleString('en-IN')}</td></tr>
        </table>
      </td></tr>`
    : `<tr><td class="k">Payments Received</td><td>No payments received yet.</td></tr>`;
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .header .logo { height: 58px; width: auto; display: block; margin: 0 auto 8px; }
    ...
    table.txn { width: 100%; border-collapse: collapse; margin: 0; }
    table.txn th { background: #f5f5f4; color: #44403c; font-size: 10px; text-transform: uppercase; letter-spacing: 0.06em; text-align: left; padding: 4px 8px; border-bottom: 1px solid #e7e5e4; }
    table.txn td { padding: 4px 8px; font-size: 11px; border-bottom: 1px solid #f0efed; }
    table.txn .r { text-align: right; }
    table.txn .txn-total td { font-weight: 600; color: #064e3b; border-top: 1px solid #d6d3d1; border-bottom: none; }
  ...
  <div class="header">
    <img class="logo" src="/Brown.png" alt="Baghaan Orchard Retreat"/>
  ...
  <table>
    ...
    ${b.rateBreakdown ? `<tr><td class="k">Payment Breakup</td><td>${esc(b.rateBreakdown)}</td></tr>` : ''}
  </table>

  <h3>Guest Details</h3>
  <table>
    <tr><td class="k">Contact Name</td><td>${esc(b.guestName)}</td></tr>
    <tr><td class="k">Contact Number</td><td>${esc(b.contactNumber)}</td></tr>
    ${b.email ? `<tr><td class="k">Email</td><td>${esc(b.email)}</td></tr>` : ''}
    ${b.companyName ? `<tr><td class="k">Company</td><td>${esc(b.companyName)}</td></tr>` : ''}
    ${b.gstNumber ? `<tr><td class="k">GST Number</td><td>${esc(b.gstNumber)}</td></tr>` : ''}
  </table>
  ...
  <table>
    ...
    ${b.remarks ? `<tr><td class="k">Remark</td><td>${esc(b.remarks)}</td></tr>` : ''}
  ...
  ${b.specialRequests ? `<p style="font-size: 11px;"><strong>Special Requests:</strong> ${esc(b.specialRequests)}</p>` : ''}

  <p class="closing">Have a Happy Holiday!</p>

  <div class="footer">
    <span>Booked by: ${esc(b.createdBy || '')}</span>
